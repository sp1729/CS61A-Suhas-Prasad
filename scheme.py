...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        return scheme_apply(op, rest.map(lambda t: scheme_eval(t, env)), env)
        # END PROBLEM 4
...
def eval_all(expressions, env):
    ...
    while expressions != nil:
        rest = scheme_eval(expressions.first, env)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(vals) != len(formals):
            raise SchemeError('Wrong num of args to function call')
        parent = Frame(self)
        while formals!=nil:
            parent.define(formals.first,vals.first)
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError("Current args are not enough")
        # END PROBLEM 3
