...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    """
    who, last0, last1 = 0,0,0  # Who is about to take a turn, 0 (first) or 1 (second)
    # BEGIN PROBLEM 5
    while score0 < goal and score1 < goal:      
        if who == 0:
            num_rolls0 = strategy0(score0, score1)
            temp0 = take_turn(num_rolls0, score1, dice)
            score0 += temp0
            
            if abs(num_rolls0-last0) == 2 and feral_hogs:
                score0 += 3
            last0 = temp0
            ...
            temp1 = take_turn(num_rolls1, score0, dice)
            score1 += temp1
            
            if abs(num_rolls1-last1) == 2 and feral_hogs:
                score1 += 3
            last1 = temp1
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    def say(score0, score1):
        last, high, current = last_score, running_high, 0
        if who == 0:
            current = score0 - last
            last = score0
        if who == 1:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    for num in range(10, 0, -1):
        score = make_averaged(roll_dice)(num, dice)
        if score >= high:
            high = score
            result = num
    return result
